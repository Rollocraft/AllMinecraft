...
public class PositionCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage("Usage: /position <create|delete|get> [name]");
            return true;
        }
        ...
        if ("create".equals(action)) {
            if (args.length < 2) {
                player.sendMessage("Usage: /position create <name>");
                return true;
            }
            ...
            try {
                ...
                player.sendMessage("Position " + name + " created.");
            ...
            if (args.length < 2) {
                player.sendMessage("Usage: /position delete <name>");
                return true;
            }
            ...
            try {
                ...
                player.sendMessage("Position " + name + " deleted.");
            } catch (SQLException e) {
                throw new RuntimeException(e);
            }
        } else if ("get".equals(action)) {
            if (args.length < 2) {
                player.sendMessage("Usage: /position get <name>");
                return true;
            }
            ...
            try {
                ...
                if (position == null) {
                    player.sendMessage("No position found with the name " + name);
                } else {
